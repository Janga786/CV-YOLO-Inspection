...
class OpenverseImageFinder:
    def __init__(self, output_dir="openverse_found_images"):
    ...
    def search_openverse_images(self, query, num_images=10):
        urls = []
        page = 1
    ...
    def find_and_download(self, queries, download_dir, target_count):
        ...
        for query in queries:
            if downloaded_count >= target_count:
                break
            print(f"\nSearching for: {query}")
            image_urls = self.search_openverse_images(query, num_images=target_count * 2)
    ...
    def run(self, target_per_category=20):
        print("=== Starting Openverse Image Finder ===")
        ...
        print("\n=== Openverse Image Finder Finished ===")

if __name__ == '__main__':
    finder = OpenverseImageFinder()
    finder.run()
