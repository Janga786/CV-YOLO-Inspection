...
class ImageFinder:
    def __init__(self, output_dir="found_images_debug"):
    ...
    def search_google_images(self, query, num_images=10):
        urls = []
        page = 0
        print(f"Searching for '{query}' on Google Images...")
        while len(urls) < num_images:
            ...
            try:
                ...
                print(f"Found {len(img_tags)} image tags on page {page + 1}.")
                if not img_tags:
                    print("No more image tags found. Stopping search.")
                    break
                for img in img_tags:
    ...
    def run(self, target_per_category=10):
        print("=== Starting Image Finder (Debug Mode) ===")
        ...
        print("\n=== Image Finder (Debug Mode) Finished ===")
